...
import { useStoreHydration } from "@/app/utils/useStoreHydration";

export default function SideBar({ currentPage }) {
  const isHydrated = useStoreHydration();
  const { searchFiles, getFileData } = useDataStore((state) => state.actions);

  let { filteredFiles, files, loading, error } = useDataStore();
  // console.log("checking:", files, loading, error);
  ...
  useEffect(() => {
    if (!isHydrated && files.length !== 0) {
      // console.log("checking effect:", files, loading, error);
