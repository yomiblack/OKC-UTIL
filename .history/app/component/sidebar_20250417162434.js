...
export default async function SideBar({ currentPage }) {
  ...
  return (
    <aside className="bg-gray-50 dark:bg-slate-900 text-slate-950 dark:text-slate-200 hidden relative md:fixed left-0 md:block border-r-2 dark:border-slate-800 border-slate-100 text-sm min-h-dvh p-4 max-w-sm ">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full p-3 border focus:outline-none focus:ring-1 focus:ring-orange-400 border-orange-500 rounded-md bg-stone-800 text-stone-50 focus:shadow-lg shadow-sm"
        />
      </div>
      ...
      {/* Filtered file list */}
      <ul className="text-black flex flex-col mt-8 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-orange-500 scrollbar-track-gray-200">
        {files.map((file) => (
          <li key={file._id}>
            <Link
              className={
