...
export default function LoginPage() {
  const getFileData = useDataStore((state) => state.getFileData('filesToPrint'));
  return (
    // Background container for the login page
    <div className="flex bg-gray-50 dark:bg-zinc-800 items-center justify-center min-h-dvh w-screen">
      {/* Form container for the login page */}
      <div
        className={`${domine.className} flex item-center justify-center min-w-fit bg-white dark:bg-zinc-900 rounded-lg mx-6 px-10 py-8 md:py-0 shadow-lg`}
      >
        {/* Flex container */}
        <div className="flex flex-col md:flex-row items-center justify-center  md:space-x-20 md:px-6">
          ...
          {/* content */}
          <form className="">
            <h2 className="text-xl text-center md:text-left font-bold  text-gray-800 dark:text-gray-100 mb-6 -mt-8 md:-mt-0">
              Log In
            </h2>
            <input
              type="email"
            ...
            <div className=" relative mb-4">
              ...
              <Link href="/home">
                <DotLottieReact
                  ...
                  loop
                  autoplay
                  className="h-8 w-8 absolute -right-8 top-1/2 transform -translate-y-1/2 cursor-not-pointer hover:scale-125 transition-transform duration-300"
                />
              </Link>
            </div>
            {/* forgot password */}
            <div className="group flex items-center space-x-3 mt-8 w-fit">
